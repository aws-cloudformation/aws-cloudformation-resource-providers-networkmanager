...
public class ValidCurrentStateCheckBaseTest extends AbstractTestBase {
    ...
    public void failedForValidAvailableState() {
        final List<Tag> tags = new ArrayList<>();
        tags.add(MOCKS.getCfnTag());
        ResourceModel model = MOCKS.modelWithoutWriteOnlyProperties(tags, AttachmentState.DELETING.toString());
        CallbackContext context =  new CallbackContext();
        when(proxyClient.client().getConnectPeer(any(GetConnectPeerRequest.class)))
                .thenReturn(MOCKS.describeResponse(tags, ConnectPeerState.DELETING.toString()));
    ...
    public void throwError() {
        ...
        ResourceModel model = MOCKS.modelWithoutWriteOnlyProperties(tags);
        CallbackContext context =  new CallbackContext();
        when(proxyClient.client().getConnectPeer(any(GetConnectPeerRequest.class))).thenThrow(ResourceNotFoundException.class);
