...
public class ListHandlerTest extends AbstractTestBase {
    ...
    public void tear_down() {
        verify(sdkClient, atLeastOnce()).listConnectPeers(any(ListConnectPeersRequest.class));
        verifyNoMoreInteractions(sdkClient);
    }

    @Test
    public void handleRequest_SimpleSuccess() {
        final ResourceModel expectedResult = MOCKS.modelWithoutWriteOnlyProperties();
        when(proxyClient.client().listConnectPeers(any(ListConnectPeersRequest.class)))
                .thenReturn(MOCKS.listResponse());

        final ProgressEvent<ResourceModel, CallbackContext> response = handler
                .handleRequest(proxy, MOCKS.request(MOCKS.modelWithoutWriteOnlyProperties()), new CallbackContext(), proxyClient, logger);

        assertThat(response).isNotNull();
        assertThat(response.getStatus()).isEqualTo(OperationStatus.SUCCESS);
        assertThat(response.getCallbackDelaySeconds()).isEqualTo(0);
        assertThat(response.getResourceModels().get(0).getConnectPeerId()).isEqualTo(expectedResult.getConnectPeerId());
        assertThat(response.getResourceModels().get(0).getConnectAttachmentId()).isEqualTo(expectedResult.getConnectAttachmentId());
    ...
    public void handleRequest_Error() {
        ...
        when(proxyClient.client().listConnectPeers(any(ListConnectPeersRequest.class))).thenThrow(exception);
