...
public class ValidCurrentStateCheckBaseTest extends AbstractTestBase {
    ...
    public void failedForValidAvailableState() {
        final Set<Tag> tags = new HashSet<>();
        tags.add(MOCKS.getCfnTag());
        ResourceModel model = MOCKS.model(tags, PeeringState.DELETING.toString());
        CallbackContext context =  new CallbackContext();
        when(proxyClient.client().getTransitGatewayPeering(any(GetTransitGatewayPeeringRequest.class)))
                .thenReturn(MOCKS.describeResponse(tags, PeeringState.DELETING.toString()));
    ...
    public void throwError() {
        ...
        ResourceModel model = MOCKS.model(tags);
        CallbackContext context =  new CallbackContext();
        when(proxyClient.client().getTransitGatewayPeering(any(GetTransitGatewayPeeringRequest.class))).thenThrow(ResourceNotFoundException.class);
    ...
    public void secondCall() {
        ...
        ResourceModel model = MOCKS.model(tags);
        CallbackContext context =  new CallbackContext();
        when(proxyClient.client().getTransitGatewayPeering(any(GetTransitGatewayPeeringRequest.class))).thenReturn(MOCKS.describeResponse(tags));
        ProgressEvent<ResourceModel, CallbackContext> response = new ValidAvailableState(proxy, MOCKS.request(model), context, proxyClient, logger)
                .run(ProgressEvent.defaultInProgressHandler(context, 0, model));
