...
import software.amazon.awssdk.services.networkmanager.model.CoreNetwork;
...
public class ValidCurrentStateCheckBase {
    ...
    public ProgressEvent<ResourceModel, CallbackContext> run(ProgressEvent<ResourceModel, CallbackContext> progress) {
        if(this.callbackContext.getAttempts() > 0) {
            logger.log("This is progress " + progress);
            return progress;
        } //skip if this not the first attempt by the lambda function
    ...
    protected CoreNetwork makeRequest() {
        GetCoreNetworkRequest getCoreNetworkRequest = GetCoreNetworkRequest.builder()
                .coreNetworkId(model.getCoreNetworkId()).build();
        return this.proxy.injectCredentialsAndInvokeV2(getCoreNetworkRequest,
                client.client()::getCoreNetwork).coreNetwork();
    }
    ...
    protected String currentState() {
        if(this._currentState != null) {
            ...
            CoreNetwork coreNetworkToDelete = this.makeRequest();
            if (coreNetworkToDelete == null) {
                return null;
            }
            return this._currentState = coreNetworkToDelete.stateAsString();
        }
    }
