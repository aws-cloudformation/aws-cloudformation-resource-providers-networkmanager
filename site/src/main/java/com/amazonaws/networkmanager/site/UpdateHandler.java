...
public class UpdateHandler extends BaseHandler<CallbackContext> {
    ...
    public ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        final UpdateSiteResponse updateSiteResponse;
        final Map<String, String> desiredResourceTags = request.getDesiredResourceTags();

        try {
            // Update site
            updateSiteResponse = updateSite(client, model, proxy);
            // Update Tags
            updateTags(client, updateSiteResponse.site().siteArn(), proxy, model, desiredResourceTags);
    ...
    private UpdateSiteResponse updateSite(final NetworkManagerClient client,
                                              final ResourceModel model,
                                              final AmazonWebServicesClientProxy proxy) {
        final UpdateSiteRequest updateSiteRequest =
                UpdateSiteRequest.builder()
                        ...
                        .build();
        return proxy.injectCredentialsAndInvokeV2(updateSiteRequest, client::updateSite);
    }
    ...
    private void updateTags(final NetworkManagerClient client,
        ...
        final Set<software.amazon.awssdk.services.networkmanager.model.Tag> desiredTags = new HashSet<>(Utils.cfnTagsToSdkTags(Utils.mergeTags(model.getTags(), desiredResourceTags)));
